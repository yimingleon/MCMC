...
% Yiming Hu, Sep, 2012
%==================================================
% I suppose, after some effort, this code is useless
...
big_boundary = [-2,2;-7,7];
boundary(1,:,:) = [-2,0;0,7];
boundary(2,:,:) = [0,2;pi,7];

regions = size(boundary);
regions = regions(1);

sig(1,:) = (big_boundary(:,2)-big_boundary(:,1))/50;
...
for i = 1:No_chain
	chains(i,:,1) = rand(1,2).*(big_boundary(:,2)-big_boundary(:,1))'+big_boundary(:,1)';
	T(i)=q^(i-1);
...
flag=0;
while(true)
	chains(1,:,1) = rand(1,2).*(big_boundary(:,2)-big_boundary(:,1))'+big_boundary(:,1)';
	for j = 1:regions
		if ([chains(1,:,1)<=bound(:,2,j)',chains(1,:,1)>=bound(:,1,j)'])
			flag=1;
			break;
		end
	end
...
%make sure that the start point is located in small regions
...
while(n < maxlength)
	...
	for i = 1:No_chain
		%here, for T=1 chain, only points in several regions are accpeted.
		%for T>1, only constraint is the bigger boundary.
		if i==1
			flag=0;
			while(true)
				new = chains(i,:,n-1)+normrnd(0,sig(i,:),boxsize);
				for j = 1:regions
					if ([new<=bound(:,2,j)',new>=bound(:,1,j)'])
						flag=1;
						break;
					end
				end
			...
			while(true) 
				new = chains(i,:,n-1)+normrnd(0,sig(i,:),boxsize);
				if ([new<=big_boundary(:,2)',new>=big_boundary(:,1)']),break,end;
			end
		end
	...
	if rand<swapPropProb
		...
		if i==1;
			flag=0;
			for j = 1:regions
				if ([chains(i+1,:,n)<=bound(:,2,j)',chains(i+1,:,n)>=bound(:,1,j)'])
					flag=1;
					break;
				end
			end
		...
		%consider swap only if the higher temperature chain is in constrained regions.
