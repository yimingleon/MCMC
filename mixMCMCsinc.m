...
for i = 1:length(subchain(:,1))
	chi2(i) = likelihood_toymodel2(subchain(i,:),t,y,variance);
end
...
while(n <= maxlength)
	...
	new_chi2 = likelihood_toymodel2(new,t,y,variance) - chi2_expect;
	...
	%coefficient = p_ref/normlikeli(i);
		...
		coefficient = p_ref/normlikeli(i)*exp(1/2*sum(q.^2./sig(i,:).^2))/exp(1/2*sum(q_ref.^2./sig(i,:).^2));%factor_now/factor_ref;%*prod(sig(i,:))/prod(sig(i_ref,:));%/exp(1/2*sum(q.^2./sig(i_ref,:)));%*2*pi*sqrt(prod(sig(i,:)));
