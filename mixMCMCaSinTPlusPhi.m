...
boundary(1,:,:) = [-2,0;0,2*pi];
boundary(2,:,:) = [0,2;pi,3*pi];
boundary(3,:,:) = [0,2;-pi,pi];
boundary(4,:,:) = [-2,0;-2*pi,0];
boundary(5,:,:) = [0,2;-3*pi,-pi];
...
for i = 1:length(subchain(:,1))
	chi2(i) = likelihood_toymodel1(subchain(i,:),t,y,variance);
end
...
while(n <= maxlength)
	...
	new_chi2 = likelihood_toymodel1(new,t,y,variance) - chi2_expect;
	...
	factor_now = exp(1/2*sum(q.^2./sig(i,:).^2));
	if i==i_ref
		coefficient = 1;
		%coefficient = p_ref/normlikeli(i)*exp(1/2*sum(q.^2./sig(i,:).^2))*2*pi*sqrt(prod(sig(i,:)));
		%coefficient = p_ref/normlikeli(i);
	else
		coefficient = p_ref/normlikeli(i)*exp(1/2*sum(q.^2./sig(i,:).^2))/exp(1/2*sum(q_ref.^2./sig(i_ref,:).^2));%*factor_now/factor_ref;%/prod(sig(i,:))*prod(sig(i_ref,:));
	%	coefficient = p_ref/normlikeli(i)*exp(1/2*sum(q.^2./sig(i,:).^2))*2*pi*sqrt(prod(sig(i,:)));
	end
	%coefficient = p_ref/normlikeli(i)*exp(1/2*sum(q.^2./sig(i,:).^2))/exp(1/2*sum(q_ref.^2./sig(i_ref,:).^2));%*factor_now/factor_ref;%/prod(sig(i,:))*prod(sig(i_ref,:));
