...
function [weight] = mixMCMC;
%function [propose] = mixMCMC;
load data.mat;
...
while(n < maxlength)
	if (~mod(n,1000))
		...
		%manuelly set some bias into this code.
		%make the first sub-chain has double the probability of normal sub-chain
		normlikeli = 1/(leng+1)*ones(1,leng);
		normlikeli(1) = 2/(leng+1);
...
%==============================
weight = subchain_distribution/maxlength;
% this shows the proportion of samples from different sub-chains.
